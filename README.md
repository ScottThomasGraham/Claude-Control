...
cd ~/Projects/Claude-Control && npm run build:sidecar && \
  CC_RDP_DEBUG=1 node scripts/live-validate.mjs 100.x.x.x user ~/.ssh/claude-control_ed25519
```
...
**Target (the Mini):** `user@100.x.x.x` (Tailscale), SSH key `~/.ssh/claude-control_ed25519`, Windows 11 build 26200, RDP already enabled (port 3389 open). RDP password is supplied at runtime via `CLAUDE_CONTROL_RDP_PASSWORD` (or the hidden prompt) — **never written to disk.**
...
node scripts/creds.mjs set 100.x.x.x user
node scripts/creds.mjs get 100.x.x.x user   # -> "set (hidden)"
```
